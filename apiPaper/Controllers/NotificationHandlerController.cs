...
public class NotificationHandlerController : ControllerBase
{
    ...
    [Route("send/topic")]
    [EnableCors("policy")]
    public async Task<ActionResult<List<Model>>> SendByTopic([FromBody] MessageInfo messageInfo)
    {
    ...
    [HttpPost]
    [EnableCors("policy")]
    public async Task<ActionResult<List<Model>>> Subscribe([FromBody] Model obj)
    {
    ...
    [HttpDelete]
    [EnableCors("policy")]
    public async Task<ActionResult<List<Model>>> Unsubscribe([FromBody] Model obj)
    {
        try
        {
    ...
    public Task<IActionResult> Get()
    {
        ...
        var el = Environment.GetEnvironmentVariable("DATABASE_URL").Split(strings, StringSplitOptions.None);
        return Task.FromResult<IActionResult>(Ok(el));
    }
}